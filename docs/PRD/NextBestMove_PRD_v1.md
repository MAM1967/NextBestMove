...
10. Action Model & State Machine (v1.0)
...
10.3 Status Model (UI Layer)

The UI presents a user-friendly status model that maps to the underlying state machine:
	•	Pending → NEW (not yet addressed)
	•	Acknowledged → SENT (responded, but follow-through may continue)
	•	Deferred → SNOOZED (intentionally postponed)
	•	Completed → DONE or REPLIED (action fully done)

This provides clearer user intent while maintaining backward compatibility with the existing state machine.

10.4 Action Source Attribution

Actions are now tracked by their source to reduce cognitive load and provide context:
	•	email — Created from email signals (AI-extracted)
	•	linkedin — Created from LinkedIn messages (future)
	•	calendar — Created from calendar events (CALL_PREP, POST_CALL)
	•	meeting_note — Created from meeting notes (AI-extracted)
	•	manual — User manually created
	•	system — System-generated by decision engine

Each action includes:
	•	source — Source type (enum)
	•	source_ref — Reference to source (e.g., email_id, meeting_note_id)
	•	intent_type — Intent classification (follow_up, reply, schedule, review, outreach, nurture)

10.5 System-Created Actions

Actions are primarily created automatically from signals to reduce manual work:
	•	Email signals → FOLLOW_UP actions (AI-extracted from email content)
	•	Meeting notes → Actions (AI-extracted action items)
	•	Calendar events → CALL_PREP (upcoming) or POST_CALL (past)
	•	Decision engine → Actions based on cadence and relationship state

All system-created actions have `auto_created = true` and appropriate `source` and `source_ref` values.
...
10.7 "Got a reply" Follow-Up Flow
...
10.8 Unified Action Card Design

All actions are displayed using a unified `UnifiedActionCard` component across:
	•	Today page (Best Action)
	•	Daily Plan page (Fast Win + regular actions)
	•	Actions page (Due View + Relationships View)

This ensures consistent visual hierarchy, interactions, and reduces cognitive load from design differences.

See Section 22 (Actions Tab) for detailed Actions page specification.
...
22. Actions Tab — Relationship-Linked Action Queue

22.1 Overview

The Actions tab provides a dedicated space for processing follow-ups and next steps in the context of relationships. It supports two mental models: "What's due today?" (Due View) and "Who do I need to work?" (Relationships View).

**Key Principles:**
	•	Relationship-first: Most actions are linked to a relationship (required for FOLLOW_UP, OUTREACH, CALL_PREP, POST_CALL)
	•	Relationship-optional: Some actions may be general business tasks (CONTENT, general NURTURE) without a specific relationship
	•	System-created, user-activated: Actions are primarily created from signals
	•	Unified design: Same action card component across all pages
	•	Fast triage: Most actions processed without opening detail views

See `docs/PRD/Actions_PRD_Refined.md` for complete specification.

22.2 Actions Page Layout

**Route:** `/app/actions`

**Components:**
	•	ActionsPage — Main page component
	•	ActionsFilterBar — Sticky filter bar with view toggle, filters
...
**Due View (Default):**
	...
	•	Grouping: Overdue → Today → Upcoming → Later → No due date
	•	Within each group: Relationship-linked actions first, then general business actions
	•	Sorting: Due date (earliest first), then created date (newest first)

**Relationships View:**
	•	Purpose: "Who do I need to work?"
	•	Grouping: By relationship (only relationship-linked actions)
	•	General business actions (CONTENT, general NURTURE) appear in separate "General Business" section
	•	Sorting: Earliest due action in group, then relationship status
	•	Max 3 actions per relationship shown (with "View all" link)

22.4 Filter Bar

**Components:**
	1.	ViewToggle — "Due" | "Relationships"
	2.	RelationshipFilter — Search-select (default: All relationships)
	3.	DueFilter — Overdue | Today | Next 7 days | This month | None | All
	4.	StatusFilter — Pending | Acknowledged | Deferred | Completed (multi-select)
	5.	SourceFilter — Email | LinkedIn | Calendar | Meeting Note | Manual | System (overflow)
	6.	IntentFilter — Follow-up | Reply | Schedule | Review | Outreach | Nurture (overflow)

22.5 Unified Action Card

**Component:** `UnifiedActionCard` (shared across Today, Daily Plan, Actions pages)

**Visual Hierarchy:**
	•	Line 1: Title (verb-led, 1 line, truncated)
	•	Line 2: Relationship anchor (name + role/company, hidden in Relationships View) OR "General Business" badge (if no relationship)
	•	Meta Row: Source icon + label (left), Due indicator (right)
	•	Status Controls: Done | Done, waiting | Snooze (pill buttons, varies by action type)
	  - Uses satisfying language: "Done" (not "Complete"), "Done, waiting" for SENT state (not "Acknowledge"), "Snooze" (not "Defer")
	  - For FOLLOW_UP actions: Keep "Done - Got reply" and "Done - No reply yet" (already good)
	•	Priority Indicator: Left border color (red/yellow/gray)
	•	Source Badge: Small badge showing source type

**Interactions:**
	•	Tap card body → ActionDetailDrawer (optional)
	•	Tap status pill → Instant optimistic update
	•	Completed actions → Fade and collapse (unless "Show completed" enabled)

22.6 System-Created Actions

Actions are automatically created from:
	•	Email signals — AI extracts recommended actions, creates FOLLOW_UP actions (relationship-required)
	•	Meeting notes — AI extracts action items from notes (usually relationship-required)
	•	Calendar events — CALL_PREP (upcoming) or POST_CALL (past) (relationship-required)
	•	Decision engine — Based on cadence and relationship state (relationship-required)
	•	Content prompts — Weekly summary generates CONTENT actions (relationship-optional)

**Relationship Requirements:**
	•	FOLLOW_UP, OUTREACH, CALL_PREP, POST_CALL — require relationship (lead_id NOT NULL)
	•	CONTENT, general NURTURE — relationship optional (lead_id nullable)

All system-created actions include:
	•	source — Source type (email, meeting_note, calendar, system, etc.)
	•	source_ref — Reference to source (email_id, meeting_note_id, etc.)
	•	intent_type — Intent classification (follow_up, reply, schedule, etc.)
	•	auto_created — true
...
**Due-Specific Empty:**
	•	Examples: "No overdue actions. You're caught up." / "Nothing due today. Work by relationship?"

**Relationships View Empty:**
	•	No relationships: CTA: Add relationship
	•	Relationships exist, no relationship-linked actions: "No open actions for relationships."
	•	Note: General business actions (if any) still appear in "General Business" section

22.8 Integration with Existing Pages
