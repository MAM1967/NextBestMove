...
All cron endpoints accept authentication in two ways:

1. **Query Parameter** (for cron-job.org):
   ```
   https://nextbestmove.app/api/cron/daily-plans?secret=YOUR_CRON_SECRET
   ```

2. **Authorization Header** (for Vercel Cron):
   ```
...
2. **URL Format:** `https://nextbestmove.app/api/cron/[endpoint]?secret=99ad993f6bcf96c3523502f028184b248b30d125a0f2964c012afa338334b0da`
3. **Method:** GET
4. **Schedule:** As specified above for each job
...
**Possible causes:**
1. **Incorrect secret:** Verify the `CRON_SECRET` in Vercel matches the one in cron-job.org URLs
2. **Missing query parameter:** Ensure `?secret=...` is included in the URL
3. **Environment variable not set:** Check that `CRON_SECRET` is set in Vercel environment variables
