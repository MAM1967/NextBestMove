...
# Script to create downgrade-to-free cron job via cron-job.org API
# Usage: ./scripts/create-downgrade-to-free-cron.sh [CRON_SECRET] [ENVIRONMENT]
...
if [ -z "$1" ]; then
  ...
  echo "Usage: ./scripts/create-downgrade-to-free-cron.sh YOUR_CRON_SECRET [staging|production]"
  exit 1
fi
...
if [ "$ENVIRONMENT" = "production" ]; then
  ...
  TITLE="Downgrade to Free Tier (Day 15)"
else
  BASE_URL="https://staging.nextbestmove.app"
  TITLE="[STAGING] Downgrade to Free Tier (Day 15)"
fi

ENDPOINT="/api/cron/downgrade-to-free"
...
echo "🚀 Creating downgrade-to-free cron job..."
...
if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "201" ]; then
  ...
  echo "   3. Verify it downgrades users with ended trials to Free tier"
  echo ""
  exit 0
else
