...
/**
 * Schedule multiple actions for a relationship, ensuring they're spread out
 * 
 * @param userId - User ID
 * @param personId - Relationship ID
 * @param actions - Array of {actionType, description, proposedDueDate}
 * @param maxActionsPerDay - Maximum actions per day (default: 2)
 * @returns Array of scheduled dates (same order as input actions)
 */
export async function scheduleMultipleActions(
  userId: string,
  personId: string,
  actions: Array<{ actionType: string; description: string; proposedDueDate: string }>,
  maxActionsPerDay: number = 2
): Promise<string[]> {
  const scheduledDates: string[] = [];
  
  // Sort actions by proposed due date (earliest first)
  const sortedActions = [...actions].sort((a, b) => {
    return new Date(a.proposedDueDate).getTime() - new Date(b.proposedDueDate).getTime();
  });
  
  // Schedule each action, ensuring no more than maxActionsPerDay per day
  for (const action of sortedActions) {
    const scheduledDate = await findNextAvailableActionDate(
      userId,
      personId,
      action.proposedDueDate,
      maxActionsPerDay
    );
